...
def InstallSettings(name, link, date):
    ...
    os.system('cd /etc/enigma2 && rm -r /etc/enigma2/blacklist bouquets.* lamedb satellites.xml userbouquet.*')
...
class CheckTimer:
    ...
    def startDownload(self, name, link, date):
        if InstallSettings(name, link, date):
            ...
            self.session.open(MessageBox, _('Nueva Lista nightupdate ') + name + _(' of ') + date + _(' updated'), MessageBox.TYPE_INFO, timeout=15)
        else:
            self.session.open(MessageBox, _('Error descarga lista'), MessageBox.TYPE_ERROR, timeout=15)
    ...
    def startTimerSetting(self):
        ...
        if OnDsl():
            print "nightupdate-Updater: CHECK FOR UPDATE"
            ...
            for date, name, link in sList:
                if name == config.pud.satname.value:
                    ...
                    if date > ConverDateBack(lastdate):
                        ...
                        print "nightupdate-Updater: nueva lista nightupdate"
                        if config.pud.just_update.value:
                            # Update without information
                            self.startDownload(self.name, self.link, ConverDate(self.date))
                        else:
                            # Auto update with confrimation
                            self.session.openWithCallback(self.CBupdate, MessageBox, _('Nueva lista nightupdate ') + name + _(' of ') + ConverDate(date) + _(' available !!' + "\n\n" + "Do you want to install the new settingslist?"), MessageBox.TYPE_YESNO, default=yesno_default, timeout=60)
                    else:
                        print "nightupdate-Updater: NO NEW UPDATE AVAILBLE"
                    break
