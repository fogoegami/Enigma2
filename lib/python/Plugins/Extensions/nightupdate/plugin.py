...
class MenuListSetting(MenuList):
    ...
    def __init__(self, list):
        ...
        font, size = skin.parameters.get("nightupdateUpdaterListFont", ('Regular',25))
        self.l.setFont(0, gFont(font, size))

class nightupdate_Updater(Screen,ConfigListScreen):

    skin =  """
        <screen name="nightupdate_Updater" position="center,center" size="710,500">
    ...
    def __init__(self, session):
        ...
        self.skinName = "nightupdate_Updater"
        self.setup_title = _("Lista canales nightupdate")
        self.setTitle(self.setup_title)
        self["description"] = Label(_("Lista Instalada") + ":\n" + "n/a")
        self["update"] = Label(_("disabled"))

        self["key_red"] = StaticText(_("Exit"))
        self["key_green"] = StaticText(_("Install"))
        self["key_yellow"] = StaticText(_("Auto"))
    ...
    def Info(self):
        ...
        if config.pud.autocheck.value:
            ...
            self['update'].setText(_("disabled"))
        if config.pud.lastdate.value == '':
            self["description"].setText(_("Lista nightupdate instalada:") + ":" + "n/a")
        else:
            self["description"].setText(_("Lista nightupdate instalada") + ":" + config.pud.satname.value + " " + config.pud.lastdate.value)

    def ListEntryMenuSettings(self, name, date, link, name1, date1):
        res = [(name, date, link, name1, date1)]
        try:
            x, y, w1, w2, h = skin.parameters.get("nightupdateUpdaterList", (15,7,420,210,40))
        except ValueError:
...
def Main(session, **kwargs):
    session.open(nightupdate_Updater)

def nightupdateSetup(menuid, **kwargs):
	if menuid == "scan":
		return [(_("Descarga lista canales"), Main, "Descarga_lista", None)]
	else:
		return []
...
def Plugins(**kwargs):
    if nimmanager.hasNimType("DVB-S"):
        return [
            PluginDescriptor(name="nightupdate", description="Descarga lista canales nightupdate", where = PluginDescriptor.WHERE_MENU, needsRestart = False, fnc=nightupdateSetup),
